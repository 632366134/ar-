...
import { checkBtnPermission, checkSysFilePermission } from "./utils/checkBtnPermission"
...
Vue.filter('changeDate', (value) => {
    ...
    return value.split(' ')[0].replace('-', '年').replace('-', '月') + '日' + ' ' + value.split(' ')[1].replace(':', '时').replace(':', '分') + '秒'
})
...
Vue.filter('timeToDateHM', (value) => {
    ...
    return value.split(' ')[0].replace('-', '年').replace('-', '月') + '日' + ' ' + time.substr(0, time.lastIndexOf(':'))
})
...
Vue.filter('timeToDate', (time) => {
    ...
    let h = (time - d * 86400) / 3600 >= 1 ? Math.floor((time - d * 86400) / 3600) : 0;
...
let firstLoad = true, mainSection = document.querySelector("#mainSection"), timer
